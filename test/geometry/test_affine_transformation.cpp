...
// All tests were made based on this simulation in GeoGebra: https://www.geogebra.org/graphing/g3mkvegy. Some rotations and displacement vectors were taken randomly and some are specific edge cases.
...
::testing::AssertionResult angles_near(double actual, double expected, double epsilon) {
  ...
  return ::testing::AssertionFailure()
         << actual << " rad is not within " << epsilon << " rad of " << expected
...
TEST_F(CoordinateSystemTestPointsInQuadrantI, IdenticalOriginTest0Rotation) {
  ...
  // The displacement vector is the (-1) * vector from the origin of the first coordinate system to the origin of the second coordinate system.
  Point<double> displacement_vector{-0.0, -0.0};
  displacement_vector = rotate(displacement_vector, rotation); 
  // We did all above tweaks so that we went from transforming just axis to transforming points between coordinate systems.
  // These values are read from the GeoGebra simulation for the second coordinate system.
  ...
  EXPECT_NEAR(arbitrary_point_second_xy_plane.x, transform_point(result, arbitrary_point_first_xy_plane).x, EPSILON) << "Expected arbitrary point in second coordinate system to be (" << arbitrary_point_second_xy_plane.x << ", " << arbitrary_point_second_xy_plane.y << ") but got (" << transform_point(result, arbitrary_point_first_xy_plane).x << ", " << transform_point(result, arbitrary_point_first_xy_plane).y << ").";
  EXPECT_NEAR(arbitrary_point_second_xy_plane.y, transform_point(result, arbitrary_point_first_xy_plane).y, EPSILON) << "Expected arbitrary point in second coordinate system to be (" << arbitrary_point_second_xy_plane.x << ", " << arbitrary_point_second_xy_plane.y << ") but got (" << transform_point(result, arbitrary_point_first_xy_plane).x << ", " << transform_point(result, arbitrary_point_first_xy_plane).y << ").";
}
