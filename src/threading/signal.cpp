...
void Signal::notify(int count) {
  ...
  SCU_UNLIKELY_THROW_IF(::syscall(SYS_futex, &_futex, FUTEX_WAKE | FUTEX_PRIVATE_FLAG, count, nullptr, nullptr, 0) < 0,
    SignalException, "Futex wake failed: "s + std::strerror(errno));
}
...
void Signal::close() {
  ...
  SCU_UNLIKELY_THROW_IF(::syscall(
    ...
    "Futex wake failed: "s + std::strerror(errno));
}
...
void Signal::wait() {
  while (SCU_UNLIKELY(!SCU_EAGER_SELECT_IS_READY())) {
    ...
    SCU_UNLIKELY_THROW_IF(error != 0 && errno != EAGAIN && errno != EINTR,
      SignalException, "Futex wait failed: "s + std::strerror(errno));
  }
}
